...
##PURPOSE: Adding climate to mean_df_subset-pops

#The purpose of this script is to add my climate variables to the mean_df_subset file
#(mean estimates for all population-level parameters) for ease of graphing clines later on
...
library(plotly)
##############################################################################
mean_df_subset <- read.csv("TPC-outputs/FinPopCodes/mean_df_subset-pops.csv") # average parameters for each population
worldclim <- read.csv("Raw-data/1-17bioclim_pops.csv") #Climate data

#Add Lat, MAT, and Seasonality params back in
mean_df_subset$lat <- c(rep(NA,length(mean_df_subset$pop)))
mean_df_subset$long <- c(rep(NA,length(mean_df_subset$pop)))
mean_df_subset$MAT <- c(rep(NA,length(mean_df_subset$pop)))
mean_df_subset$tempseason <- c(rep(NA,length(mean_df_subset$pop)))
mean_df_subset$maxwarmest <- c(rep(NA,length(mean_df_subset$pop)))#max temp of warmest month, bio_5
mean_df_subset$meanwarmest <- c(rep(NA,length(mean_df_subset$pop)))#mean temp of warmest quarter, bio_10
mean_df_subset$mincoldest <- c(rep(NA,length(mean_df_subset$pop))) #min temp of coldest month, bio_6


#match temperature variable to appropriate population based on ORIGINAL POPULATION CODE (not new pop code- doesn't really matter either way, but just so ya know)
...
#Edit temp variables to reflect actual vals, since worldclim converts them to weird forms
...
write.csv(mean_df_subset, "Processed-data/mean_df_subsetwithclim1.csv", row.names = FALSE)
